...
def render_comparison_table(
    ...
    *,
    ...
    numeric_by_field = _coerce_numeric_columns(aggregated, entity_slugs=entity_slugs, fields=fields)
...
def compute_leader_changes(
    ...
    *,
    ...
    """
    cost_fields_set = set(cost_fields)
    numeric = _coerce_numeric_columns(aggregated, entity_slugs=entity_slugs, fields=fields)
    current_leaders: dict[str, str | None] = {}
    for col in fields:
        first, _second = _rank_top_two(numeric.get(col, {}), lower_is_better=col in cost_fields_set)
        current_leaders[col] = first
    prior_leaders = _read_prior_leaders(vault, path, fields=fields, git_runner=git_runner)
    ...
    for col in fields:
        ...
        if was != now and now is not None:
            if was is None:
                bullets.append(f"- **`{col}`**: first leader recorded — [[{now}]].")
            else:
                bullets.append(f"- **`{col}`**: [[{now}]] took #1 from [[{was}]].")
    if not bullets:
        return None
    return "\n".join(bullets)
...
def _read_prior_leaders(
    ...
    *,
    ...
    try:
        ...
        return None
    result = runner(["git", "-C", str(repo_root), "show", f"HEAD:{rel.as_posix()}"])
