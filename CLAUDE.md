...
The **daily completeness sentinel** closes that structural gap. Once per day (default `0 14 * * *` UTC = 11 AM Brasília, configurable via `.github/workflows/daily-completeness.yml` — positioned one hour after the Conflict Resolver's 10 AM Brasília cron so any late-recovery requeues have already landed by the time the sentinel evaluates), it runs `wikipilot daily-completeness-check`, which:
