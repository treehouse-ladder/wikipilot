...
The Daily Research routine's editorial top section, generated by the opus `daily-brief-curator` (see `docs/runbook.md` "Reading the daily brief"). The transclusion below resolves to the most recent `wiki/reports/<DATE>.md`'s `## Today's brief` block.

```dataviewjs
// Find the most recent daily report, transclude its `## Today's brief` block.
const reports = dv.pages('"reports"')
  .where((p) => p.routine === "daily_research")
  .sort((p) => p.last_updated, "desc");
if (reports.length === 0) {
  dv.paragraph("_no daily reports yet — run the routine to populate this section._");
} else {
  const latest = reports[0];
  dv.paragraph(`> Latest report: ${latest.file.link} (${latest.last_updated})`);
  dv.paragraph(`![[${latest.file.name}#Today's brief]]`);
  dv.paragraph(`![[${latest.file.name}#Leader changes]]`);
}
```
...
The two persistent comparison pages the daily runner refreshes. Open either to see the leader-highlighted table + per-column "what good-at-this means for me" legend.

```dataview
TABLE WITHOUT ID
  file.link AS "Snapshot",
  last_updated AS "Refreshed",
  length(comparison_of) AS "Models",
  length(compare_fields) AS "Fields"
FROM "comparisons"
WHERE file.name = "cost-comparison" OR file.name = "benchmark-leaders"
SORT file.name ASC
```
